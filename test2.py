...
# Signature recognition (unchanged)
class SignatureRecognizer:
    ...
    def load_database(self):
        ...
        for fn in os.listdir(self.database_path):
            ...
            try:
                with Image.open(path): pass
            except:
                continue
            skel, minutiae = self.process_image(path)
...
class DeepFaceRecognizer:
    ...
    def recognize(self,qpath):
        ...
        for name,imgs in self.db.items():
            ...
            for img in imgs:
                try:
                    r=DeepFace.verify(img1_path=qpath,img2_path=img,model_name=self.model_name,enforce_detection=self.enforce_detection)
...
class SignatureGUI:
    ...
    def _build_ui(self):
        # Title
        Label(self.root,text="Signature + Face Recognition",font=("Segoe UI",18,"bold"),bg="#f0f0f0").grid(row=0,column=0,columnspan=4,pady=10)
        # Buttons
        Button(self.root,text="Load Signature",bg="#4CAF50",fg="white",command=self._load_sig).grid(row=1,column=0,padx=5)
        Button(self.root,text="Load Face",bg="#FF9800",fg="white",command=self._load_face).grid(row=1,column=1,padx=5)
        self.btn_compare=Button(self.root,text="Compare",bg="#2196F3",fg="white",state="disabled",command=self._compare)
        self.btn_compare.grid(row=1,column=2,padx=5)
        # Sliders
        Label(self.root,text="Face weight",bg="#f0f0f0").grid(row=2,column=0)
        self.s_face=Scale(self.root,from_=0,to=1,resolution=0.05,orient=HORIZONTAL,command=self._update_w_face,length=150)
        self.s_face.set(self.w_face); self.s_face.grid(row=2,column=1)
        Label(self.root,text="Sig weight",bg="#f0f0f0").grid(row=2,column=2)
        self.s_sig=Scale(self.root,from_=0,to=1,resolution=0.05,orient=HORIZONTAL,command=self._update_w_sig,length=150)
        self.s_sig.set(self.w_sig); self.s_sig.grid(row=2,column=3)
        # Images stacked vertically
        self.lbl_sig=Label(self.root,bg="#f0f0f0"); self.lbl_sig.grid(row=3,column=0,columnspan=4,pady=5)
        self.lbl_face=Label(self.root,bg="#f0f0f0"); self.lbl_face.grid(row=4,column=0,columnspan=4,pady=5)
        # Result and plot
        self.lbl_res=Label(self.root,text="Awaiting input...",font=("Segoe UI",12),bg="#f0f0f0")
        self.lbl_res.grid(row=5,column=0,columnspan=4,pady=10)
        self.frm_plot=Frame(self.root,bg="#f0f0f0"); self.frm_plot.grid(row=6,column=0,columnspan=4)
    ...
    def _load_sig(self):
        ...
        im=Image.fromarray(skel); im.thumbnail((400,400))
    ...
    def _load_face(self):
        ...
        im=Image.open(p); im.thumbnail((400,400))
    ...
    def _compare(self):
        ...
        maxs=max(sc.values()) if sc else 1
        sc={n:v/maxs for n,v in sc.items()}
        ...
        if cmb:
            ...
            self.lbl_res.config(text=txt); self._plot(cmb)
        else: self.lbl_res.config(text="No match")

    def _plot(self,c):
        fig=Figure(figsize=(5,3),dpi=100); ax=fig.add_subplot(111)
        names,vals=zip(*c.items()); ax.bar(names,vals); ax.set_ylim(0,1); ax.set_ylabel("Conf"); ax.tick_params(axis='x',rotation=45)
